...
void main() {
    ...
    float specularFactor = pow(cosCameraToReflection, shininess) > 0.5 ? 1 : 0;
    float diffuseFactor = cosLightToNormal > 0.5 || specularFactor == 1 ? 1 : 0.5;

    color = ambientColor
        + litFactor * visibility * lightColor
        * (diffuseColor * diffuseFactor + specularColor * specularFactor);
}
